...
本报告记录一次有限算力下的完整训练实践：在 RTX 4060 8GB 上从头训练 17.23M 参数 TinyStories Base，再完成 Instruction SFT、DPO、RSFT 和统一评测。重点不是提出一个通用大模型方案，而是说明各阶段怎样运行、模型发生了什么变化、哪些指标有效，以及哪些尝试没有成功。
...
本次实践表明，一张消费级显卡足以让初学者真正走完从 tokenizer、Base 预训练到对齐和评测的流水线，而不是只停留在理论或 API 调用。在这个受控实验中，缺少广泛世界知识的 17M Base 仍学会了有限的故事域指令协议，前提是任务可构造、可验证。Instruction SFT 是最明显的行为变化来源；严格 DPO-v2 提供了一个局部改进，也让 DPO-v1 的代理奖励过度优化问题更容易被观察和理解。
